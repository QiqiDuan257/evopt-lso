function y = evoL1_RF_Ellipsoid2(X)
persistent RM sv;
[funcDim, popSize] = size(X);
if isempty(RM)  || (size(RM, 1) ~= funcDim) || (size(RM, 2) ~= funcDim)
    SRM = load(sprintf('./.evoRatationShift/evoRF_Ellipsoid_R%d.mat', funcDim));
    RM = SRM.RM;
...
if isempty(sv) || (size(sv, 1) ~= funcDim) || (size(sv, 2) ~= popSize)
    SSV = load(sprintf('./.evoRatationShift/evoRF_Ellipsoid_s%d.mat', funcDim));
    sv = SSV.sv;
...
y = evoFuncEllipsoid(RM * (X - sv)) + evoFuncReg1Norm(X);
end
